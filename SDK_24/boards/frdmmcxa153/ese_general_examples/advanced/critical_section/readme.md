Overview
========
Demonstrates what happens if critical sections are not properly protected from pre-emption.

The application prints a line in a terminal application and should display the same number between 0 and 9 on one line. However, the critical section (copying the global buffer in main to a local buffer before printing) is not protected from pre-emption. This means that if the timing is right, the printed line might show different numbers.
